    ...
    <section id="pricing" style={{ background:t.bg, paddingTop:110, paddingBottom:120, position:"relative", overflow:"hidden", transition:"background 0.4s ease" }}>
      ...
      <div style={{ maxWidth:1080,margin:"0 auto",paddingInline:24,position:"relative",zIndex:1 }}>
        <motion.div initial={{ opacity:0,y:20 }} whileInView={{ opacity:1,y:0 }}
          ...
          <span style={{ display:"inline-block",fontFamily:"'DM Mono',monospace",fontSize:"0.65rem",letterSpacing:"0.2em",textTransform:"uppercase",color:t.textFaint,marginBottom:16 }}>Pricing</span>
          ...
          <p style={{ fontFamily:"'Outfit',sans-serif",fontSize:"1rem",color:t.textMuted,marginTop:12,lineHeight:1.6 }}>
        ...
        <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(280px,1fr))",gap:16,alignItems:"stretch" }}>
          {plans.map((plan,i) => (
            ...
            >
              {plan.highlight && (
                <span style={{ display:"inline-block",fontFamily:"'DM Mono',monospace",fontSize:"0.6rem",letterSpacing:"0.2em",textTransform:"uppercase",color:isDark?"#050505":"#ffffff",background:isDark?"rgba(0,0,0,0.08)":"rgba(255,255,255,0.15)",borderRadius:4,padding:"3px 9px",marginBottom:14,alignSelf:"flex-start" }}>
                  Most Popular
                </span>
              )}
              <h3 style={{ fontFamily:"'Outfit',sans-serif",fontWeight:800,fontSize:"1.3rem",letterSpacing:"-0.03em",color:plan.highlight?(isDark?"#050505":"#ffffff"):t.text,margin:"0 0 6px" }}>{plan.name}</h3>
              <p style={{ fontFamily:"'Outfit',sans-serif",fontSize:"0.8rem",color:plan.highlight?(isDark?"rgba(0,0,0,0.45)":"rgba(255,255,255,0.55)"):t.textMuted,margin:"0 0 26px",lineHeight:1.5 }}>{plan.description}</p>
              <div style={{ display:"flex",alignItems:"baseline",gap:4,marginBottom:28 }}>
                <span style={{ fontFamily:"'Outfit',sans-serif",fontWeight:800,fontSize:"2.8rem",letterSpacing:"-0.05em",lineHeight:1,color:plan.highlight?(isDark?"#050505":"#ffffff"):t.text }}>₹{plan.price}</span>
                <span style={{ fontFamily:"'DM Mono',monospace",fontSize:"0.72rem",color:plan.highlight?(isDark?"rgba(0,0,0,0.4)":"rgba(255,255,255,0.45)"):t.textFaint,letterSpacing:"0.05em" }}>/mo</span>
              </div>
              ...
              <ul style={{ listStyle:"none",padding:0,margin:"0 0 32px",display:"flex",flexDirection:"column",gap:13,flex:1 }}>
                {plan.features.map((feat) => (
                  <li key={feat} style={{ display:"flex",alignItems:"flex-start",gap:10 }}>
                    ...
                    <span style={{ fontFamily:"'Outfit',sans-serif",fontSize:"0.83rem",fontWeight:500,color:plan.highlight?(isDark?"rgba(0,0,0,0.7)":"rgba(255,255,255,0.75)"):t.textMuted,lineHeight:1.4 }}>{feat}</span>
                  </li>
                ))}
              </ul>
              <button onClick={onOpenModal} style={{
                width:"100%",padding:"13px 0",borderRadius:9999,
                fontFamily:"'Outfit',sans-serif",fontWeight:700,fontSize:"0.84rem",letterSpacing:"0.02em",
